...
class PlayerFactory {
    static func getPlayer(type: PlayerType) -> Player? {
        ...
        case .Female:
            return Player(texture: SKTexture(imageNamed: "womanGreen_gun"), color: SKColor.green, size: CGSize(width: 51, height: 43), health: 10, moveSpeed: 4, weapon: WeaponFactory.getWeapon(type: .Pistol))
        }
    }
}
